...
void cCamera::renderView(const int a_windowWidth, const int a_windowHeight,
	OVR::Matrix4f projectionMatrici, ovrPosef eyePoses, ovrVector3f	cameraPosition,
	const int a_imageIndex)
{
	...
	// compute global pose
	computeGlobalCurrentObjectOnly(false);
	...
	if (a_imageIndex == CHAI_MONO || a_imageIndex == CHAI_STEREO_DEFAULT)
	{
		...
		glLoadIdentity();
		//glMultMatrixf(&(projectionMatrici.Transposed().M[0][0]));
		...
		OVR::Quatf l_Orientation = OVR::Quatf(eyePoses.Orientation);
		OVR::Matrix4f l_ModelViewMatrix = OVR::Matrix4f(l_Orientation.Inverted());
		glMultMatrixf(&(l_ModelViewMatrix.Transposed().M[0][0]));

		// Translation due to positional tracking (DK2) and IPD...
		glTranslatef(-eyePoses.Position.x, -eyePoses.Position.y, -eyePoses.Position.z);
