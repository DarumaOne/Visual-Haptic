...
struct Scene  
{
    ...
    Scene(int reducedVersion) : num_models(0) // Main world
    {
        ...
        // Construct geometry
        Model * m = new Model(Vector3f(0,0,-10),generated_texture[2]);  // Red box
		m->AddSolidColorBox(0, 0, 0, +1.0f, +1.0f, 1.0f, Model::Color(255, 0, 0));
        m->AllocateBuffers(); Add(m);

		m = new Model(Vector3f(-2, 0, -10), generated_texture[2]);  // Green box
		m->AddSolidColorBox(0, 0, 0, +1.0f, +1.0f, 1.0f, Model::Color(0, 255, 0));
		m->AllocateBuffers(); Add(m);

		m = new Model(Vector3f(2, 0, -10), generated_texture[2]);  // Blue box
		m->AddSolidColorBox(0, 0, 0, +1.0f, +1.0f, 1.0f, Model::Color(0, 0, 255));
		m->AllocateBuffers(); Add(m);
